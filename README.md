...
> **Note:** If you are connecting to real AWS resources (S3, SQS, Bedrock), you must still provision the AWS infrastructure (see **Step 1** under _Docker & AWS Setup_) and populate your `.env` file first.
...
3. Once the deployment successfully completes, fetch the necessary outputs to populate your `.env` file:
   ...
   _You will map these outputs to variables like `S3_BUCKET_INPUT`, `SQS_PROCESSING_QUEUE_URL`, etc., in the next step._
...
_How it works locally:_ Instead of relying on local mock services (like LocalStack or SQLite), your local 3-container setup uses your exact `aws-infrastructure.yaml` configuration. When a file is uploaded, it goes straight to the real S3 bucket. The message is pushed to the real SQS queue, picked up by your local worker container, and LLM reasoning uses the real AWS Bedrock models.
