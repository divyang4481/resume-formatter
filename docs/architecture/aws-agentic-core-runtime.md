...
2. **Asynchronous Worker (Fargate)**: An ECS task continuously polls an SQS Queue. Upon receiving a job, it parses the document through the configured Docling parser service, validates if it is a resume, and maps the output to a canonical format.
